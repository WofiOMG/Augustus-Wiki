...
**Intave13:**
Cancels or reduces knockback by manipulating movement packets specifically tailored for Intave anti-cheat.
...
JumpDelay also has this setting:
...
**CancelType:**
 None / OnlyOnGround / OnlyInAir / Always
PacketCount: 6
...
**S32PacketConfirmTransaction**
CancelType: None / OnlyOnGround / OnlyInAir / Alway
PacketCount: 1

**S48PacketResourcePackSend**
CancelType: None / OnlyOnGround / OnlyInAir / Alway
PacketCount:
Amount of packets to cancle.
...
**Expert:**
 More advanced, uses hurttime with pushmottion depending on the hurttime.

 **Expert Hurttimes**
 There are 10 diffrent modes, each giving diffrent values depending on the hurttime.
...
**Packets To Cancel:**
List of packets to cancel (prevent from being sent/processed) to reduce knockback or avoid server detection. The common ones here are:

500PacketKeepAlive
S32PacketConfirmTransaction
548PacketResourcePackSend

**For each packet type (e.g., 500PacketKeepAlive):**
**CancelType: **
When to cancel the packet:

**None: Never cancel**
OnlyOnGround: Cancel only if player is on ground
OnlyInAir: Cancel only if player is in air
Always: Always cancel

**PacketCount:**
 Number of packets to cancel at once or per tick (usually a small integer like 6 or 1).